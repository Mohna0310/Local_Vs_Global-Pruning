...
seed_numbers = [42]
model_type = 'bert'
absa_type = 'crf'      # 'crf', 'gru', 'san', 'tfm', 'linear'
tfm_mode = 'finetune'
fix_tfm = 0
pruning = 'local'
task_name = 'laptop14'
...
for run_id, seed in enumerate(seed_numbers):
    command = "python main.py --model_type %s --absa_type %s --tfm_mode %s --fix_tfm %s " \
              ...
              "--overwrite_output_dir --eval_all_checkpoints --MASTER_ADDR localhost --MASTER_PORT 28512 --pruning %s" % (
        model_type, absa_type, tfm_mode, fix_tfm, task_name, task_name, train_batch_size, warmup_steps, seed, overfit, pruning)
