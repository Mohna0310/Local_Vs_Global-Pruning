...
def label_rest_xml(fn, output_fn, corpus, label):
    ...
    for zx, sent in enumerate(root.iter("sentence") ) :
        ...
        start, end=-1, -1
        # print("TEXT:", sent.find('text').text)
...
def evaluate(runs, data_dir, model_dir, domain, pruning, command, template):
    ...
    for r in range(runs):
        ...
        if pruning == 'local':
            model=torch.load(model_dir+domain+str(r)+"_local.pt")  
        else:
            model=torch.load(model_dir+domain+str(r)+"_global.pt") 
...
if __name__ == "__main__":    
    ...
    if args.pruning == 'local':
        for pr_per in list(range(10,91,10))+[95]:
            print(f"Local Pruning : {pr_per} %")
            abs_path = args.model_dir+"local_"+ args.domain+"/"+str(pr_per)+'%/'
            evaluate(args.runs, args.data_dir, abs_path, args.domain, args.pruning, command, template)
            print()
            print("*"*51)
            print()
        ...
        for pr_per in list(range(10,91,10))+[95]:
            ...
            abs_path = args.model_dir+"global_"+ args.domain+"/"+str(pr_per)+'%/'
            evaluate(args.runs, args.data_dir, abs_path, args.domain, args.pruning, command, template)
            print()
            print("*"*51)
            print()
