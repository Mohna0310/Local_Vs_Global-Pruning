...
def train(args, train_dataset, model, tokenizer, parameters_to_prune, prun_ratio, prune_type):
    """ Train the model """
    ...
    for _ in train_iterator:
        ...
        if prune_type == 'global':
            ...
            prune.global_unstructured(parameters_to_prune, pruning_method = prune.L1Unstructured, amount = 0.95)
            ...
            for j,i in enumerate(parameters_to_prune):
                modu,parm = i
                prune.l1_unstructured(modu, name=parm, amount= prun_ratio[j])
                prune.remove(modu, parm)
...
def main():
    ...
    print("Check : ",args.no_cuda, torch.cuda.is_available())
    ...
    for name,mod in list(model.named_modules())[1:]:
        ...
        if dict().keys() == keys:
            ...
            if 'bias' in keys:
                #print('bias', name, name + '.bias')
                parameters_to_prune.append((mod,'bias'))
                parameters.append((model.state_dict()[name + '.bias'] !=0.).sum().item())
                #parameters[name + '.bias'] = (model.state_dict()[name + '.bias'] !=0.).sum().item()
            
            if 'weight' in keys: 
                #print('weigght', name, name + '.weight')
                parameters_to_prune.append((mod,'weight'))
                parameters.append((model.state_dict()[name + '.weight'] !=0.).sum().item())
                #parameters[name + '.weight'] = (model.state_dict()[name + '.weight'] !=0.).sum().item()
                
            if ('in_proj_weight' in keys ):
                parameters_to_prune.append((mod,'in_proj_weight'))
                parameters.append((model.state_dict()[name + '.in_proj_weight'] !=0.).sum().item())
                
            if ('in_proj_bias' in keys):
                parameters_to_prune.append((mod,'in_proj_bias'))
                parameters.append((model.state_dict()[name + '.in_proj_bias'] !=0.).sum().item())
            
                
            
    # print(len(parameters_to_prune),"Parameters to prune : "+ '*'*25)
    # print(parameters_to_prune) 
    
    layer_per = np.array(parameters)/sum(parameters)
    
    arr = np.array(parameters)
    idx = arr.argsort()[::-1]
    
    net_prun_amount = 0.10  # Pruning amount #
    prun_ratio = [0.] *len(parameters)
    
    params = sum(parameters)
    
    if (args.absa_type == 'linear') or (args.absa_type == 'crf'):
        n_idx = 74
        n1 = 24
        
    elif args.absa_type == 'tfm':
        n_idx = 78
        n1 = 27
        
    elif args.absa_type == 'san':
        n_idx = 76
        n1 = 25
        
    elif args.absa_type == 'gru':
        n_idx = 75
        n1 = 24
        
        
        
    for i,j in enumerate(idx[:n_idx]):
        
        if net_prun_amount * 100 == 10:
            
            ratio = 0.02/(len(idx[:n_idx])-1-n1)
            ratio1 = 0.06/ n1
            if i == 0:
                prun_ratio[j] = (0.02 * params)/ parameters[j]
                continue
            
            elif i in range(1,n1+1):
                if ratio1 < layer_per[j]:
                    prun_ratio[j] = (ratio1 * params)/parameters[j]
                
            else:
                if ratio < layer_per[j]:
                    prun_ratio[j] = (ratio * params)/parameters[j]
            
            
        elif net_prun_amount * 100 == 20:
            
            ratio = 0.07/(len(idx[:n_idx])-1-n1)
            ratio1 = 0.10/ n1
            if i == 0:
                prun_ratio[j] = (0.03 * params)/ parameters[j]
                continue
        ...
        elif net_prun_amount * 100 == 30:
            
            ratio = 0.05/(len(idx[:n_idx])-1-n1)
            ratio1 = 0.21/ n1
            if i == 0:
                prun_ratio[j] = (0.04 * params)/ parameters[j]
                continue
        ...
        elif net_prun_amount * 100 == 40:
            
            ratio = 0.11/(len(idx[:n_idx])-1-n1)
            ratio1 = 0.25/ n1
            if i == 0:
                prun_ratio[j] = (0.04 * params)/ parameters[j]
                continue
        ...
        elif net_prun_amount * 100 == 50:
            
            ratio = 0.13/(len(idx[:n_idx])-1-n1)
            ratio1 = 0.30/ n1
            if i == 0:
                prun_ratio[j] = (0.07 * params)/ parameters[j]
                continue
        ...
        elif net_prun_amount * 100 == 60:
            
            ratio = 0.135/(len(idx[:n_idx])-1-n1)
            ratio1 = 0.375/ n1
            if i == 0:
                prun_ratio[j] = (0.09 * params)/ parameters[j]
                continue
        ...
        elif net_prun_amount * 100 == 70:
            
            ratio = 0.16/(len(idx[:n_idx])-1-n1)
            ratio1 = 0.41/ n1
            if i == 0:
                prun_ratio[j] = (0.13 * params)/ parameters[j]
                continue
        ...
        elif net_prun_amount * 100 == 80:
            
            ratio = 0.20/(len(idx[:n_idx])-1-n1)
            ratio1 = 0.45/ n1
            if i == 0:
                prun_ratio[j] = (0.15 * params)/ parameters[j]
                continue
        ...
        elif net_prun_amount * 100 == 90:
            
            ratio = 0.25/(len(idx[:n_idx])-1-n1)
            ratio1 = 0.48/ n1
            
            if i == 0:
                prun_ratio[j] = (0.17 * params)/ parameters[j]
                continue
        ...
        elif net_prun_amount * 100 == 95:
            
            ratio = 0.25/(len(idx[:n_idx])-1-n1)
            ratio1 = 0.51/ n1
            if i == 0:
                prun_ratio[j] = (0.19 * params)/ parameters[j]
                continue
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, parameters_to_prune, prun_ratio, prune_type)
